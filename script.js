document.addEventListener('DOMContentLoaded', () => {
    ...
    let grid = Array(GRID_COLS).fill().map(() => Array(GRID_ROWS).fill(null));
    let plants = [];
    let zombies = [];
    ...
    function setupEventListeners() {
        ...
        pauseRestartBtn.addEventListener('click', () => { togglePause(); startNewGame(); });
        pauseQuitBtn.addEventListener('click', () => { togglePause(); showStartScreen(); });
        restartBtn.addEventListener('click', () => { hideModal('game-over-modal'); startNewGame(); });
        homeBtn.addEventListener('click', () => { hideModal('game-over-modal'); showStartScreen(); });
    ...
    function startNewGame() {
        ...
        sunInterval = setInterval(() => { if (gameActive && !gamePaused) createSun(); }, 10000);
        setTimeout(() => { if (gameActive) createSun(); }, 2000);
    ...
    function resetGameState() {
        ...
        plants = [];
        zombies = [];
        grid = Array(GRID_COLS).fill().map(() => Array(GRID_ROWS).fill(null));
    ...
    function createSun(x = null, y = null) {
        ...
        setTimeout(() => { if (sun.parentElement) { sun.style.opacity = 0; setTimeout(() => sun.remove(), 500); } }, 8000);
    }
    ...
    function spawnPlant(type, col, row, loadedHp = null) {
        ...
        const plantObj = { type, element: plant, col, row, hp: loadedHp || stats.hp, maxHp: stats.hp, hpBar, intervals: [], armed: type !== 'potatomine', chomperCooldown: false };
        plants.push(plantObj);
        grid[col][row] = plantObj;

        // Plant behaviors
        if (type === 'sunflower') {
            const int = setInterval(() => { if (gameActive && !gamePaused) createSun(pos.x + 40, pos.y); }, 8000);
            plantObj.intervals.push(int);
        } else if (type === 'peashooter' || type === 'snowpea') {
            const peaType = type === 'snowpea' ? 'pea-ice' : 'pea-normal';
            const int = setInterval(() => { if (gameActive && !gamePaused) checkAndShoot(plantObj, peaType); }, 1200);
            plantObj.intervals.push(int);
            ...
            const int = setInterval(() => {
                ...
                if (target) {
                    ...
                    setTimeout(() => { plantObj.chomperCooldown = false; plant.style.filter = ''; }, 25000);
                }
            }, 500);
            plantObj.intervals.push(int);
        } else if (type === 'potatomine') {
            plant.style.opacity = '0.5';
            setTimeout(() => { if (plant.parentElement) { plantObj.armed = true; plant.style.opacity = '1'; createFloatingText('已武装!', pos.x + 'px', pos.y + 'px', 'yellow'); } }, 12000);
            const int = setInterval(() => {
                ...
                if (target) {
                    ...
                    zombies.filter(z => z.row === plantObj.row && Math.abs(z.left - pos.x) < 120).forEach(z => { z.hp = 0; killZombie(z); });
                    killPlant(plantObj);
                }
            }, 100);
            plantObj.intervals.push(int);
            ...
            setTimeout(() => {
                ...
                zombies.filter(z => Math.hypot(z.left - pos.x, z.top - pos.y) < 180).forEach(z => { z.hp = 0; killZombie(z); });
                killPlant(plantObj);
            }, 1200);
        }
    }
    ...
    function shootPea(x, y, row, typeClass) {
        ...
        const interval = setInterval(() => {
            if (!gameActive) { clearInterval(interval); pea.remove(); return; }
            if (gamePaused) return;
            currentX += 10;
            ...
            if (hit) {
                hit.hp -= 30;
                if (typeClass === 'pea-ice' && !hit.frozen) { hit.frozen = true; hit.speed *= 0.5; hit.element.style.filter = 'brightness(1.3) hue-rotate(180deg)'; }
                updateHealthBar(hit.hpBar, hit.hp);
                if (hit.hp <= 0) killZombie(hit);
                clearInterval(interval);
                pea.remove();
            }
            if (currentX > 1200) { clearInterval(interval); pea.remove(); }
        }, 16);
    }
    ...
    function startWave() {
        ...
        const spawnInt = setInterval(() => {
            if (!gameActive) { clearInterval(spawnInt); return; }
            if (gamePaused) return;
    ...
    function spawnZombie(row, type, startX = 1100, loadedHp = null) {
        ...
        const zombieObj = { type, element: zombie, row, left: startX, top: topY, hp: loadedHp || stats.hp, maxHp: stats.hp, speed: stats.speed, hpBar, eating: false, frozen: false, intervals: [] };
            ...
            if (target) {
                if (!zombieObj.eating) {
                    ...
                    const eatInt = setInterval(() => {
                        if (!gameActive || gamePaused || !target.element?.parentElement) { clearInterval(eatInt); zombieObj.eating = false; return; }
                        target.hp -= 25;
                        updateHealthBar(target.hpBar, target.hp);
                        if (target.hp <= 0) { killPlant(target); clearInterval(eatInt); zombieObj.eating = false; }
                    }, 800);
    ...
    function updateHealthBarPosition(hb, x, y) {
        if (hb?.bar) { hb.bar.style.left = (x + 15) + 'px'; hb.bar.style.top = (y - 8) + 'px'; }
    }
