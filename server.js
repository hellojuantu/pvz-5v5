...
try { if (fs.existsSync(DATA_FILE)) persistedData = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8')); } catch (e) {}
Object.entries(persistedData.sessions || {}).forEach(([k, v]) => sessions.set(k, v));

function saveData() {
    const sessObj = {}; sessions.forEach((v, k) => { sessObj[k] = v; });
    const roomStates = {};
    rooms.forEach((room, id) => {
        if (room.state === 'playing' || room.state === 'paused') {
            roomStates[id] = {
                id: room.id, mode: room.mode, hostName: room.hostName, maxWaves: room.maxWaves,
                sunCount: room.sunCount, brainCount: room.brainCount, waveNumber: room.waveNumber, state: room.state,
                ...
                zombies: room.zombies.map(z => ({ id: z.id, type: z.type, row: z.row, x: z.x, hp: z.hp, maxHp: z.maxHp, speed: z.speed, slowed: z.slowed })),
...
const CELL_SIZE = 110, GRID_COLS = 9, GRID_ROWS = 5;
...
class GameRoom {
    constructor(id, mode, hostId, hostName, maxWaves = MAX_WAVES_DEFAULT) {
        ...
        this.grid = Array(GRID_COLS).fill().map(() => Array(GRID_ROWS).fill(null));
        this.plants = [];
        this.zombies = [];
        this.projectiles = [];
    ...
    restoreFrom(saved) {
        ...
        this.plants.forEach(p => { this.grid[p.col][p.row] = p; });
    }
    ...
    addBot(team) {
        const teamArray = team === 'plants' ? this.plantPlayers : this.zombiePlayers;
        if (teamArray.length >= this.mode) return null;
        const bot = { id: 'bot_' + Date.now() + Math.random().toString(36).substr(2, 4), name: ['小明AI', '小红AI', '小刚AI'][Math.floor(Math.random() * 3)], isBot: true, socket: null };
        teamArray.push(bot);
        return bot;
    }

    findPlayerByOderId(oderId) { return [...this.plantPlayers, ...this.zombiePlayers].find(p => p.oderId === oderId); }
    ...
    removePlayer(playerId, forceLeave = false) {
        ...
        if (wasPlaying && !forceLeave) {
            ...
            if (realPlayers.length < this.plantPlayers.length + this.zombiePlayers.length - this.plantPlayers.filter(p => p.isBot).length - this.zombiePlayers.filter(p => p.isBot).length) {
                this.pauseGame('玩家离开');
            }
        }
    }
    
    isFull() { return this.plantPlayers.length >= this.mode && this.zombiePlayers.length >= this.mode; }
    getInfo() { return { id: this.id, mode: this.mode, maxWaves: this.maxWaves, hostName: this.hostName, plants: this.plantPlayers.length, zombies: this.zombiePlayers.length, state: this.state }; }
    getPlayerList() { return { plants: this.plantPlayers.map(p => ({ id: p.id, name: p.name, isBot: p.isBot })), zombies: this.zombiePlayers.map(p => ({ id: p.id, name: p.name, isBot: p.isBot })) }; }
    getGameState() { return { sunCount: this.sunCount, brainCount: this.brainCount, waveNumber: this.waveNumber, maxWaves: this.maxWaves, plants: this.plants.map(p => ({ type: p.type, col: p.col, row: p.row, hp: p.hp, maxHp: p.maxHp, armed: p.armed })), zombies: this.zombies.map(z => ({ id: z.id, type: z.type, row: z.row, x: z.x, hp: z.hp, maxHp: z.maxHp, slowed: z.slowed })) }; }

    broadcastTo(team, event, data) { (team === 'plants' ? this.plantPlayers : this.zombiePlayers).forEach(p => { if (p.socket?.connected) p.socket.emit(event, data); }); }
    broadcast(event, data) { [...this.plantPlayers, ...this.zombiePlayers].forEach(p => { if (p.socket?.connected) p.socket.emit(event, data); }); }
    sendChat(senderName, message) { const chatMsg = { sender: senderName, message, time: Date.now() }; this.chatHistory.push(chatMsg); if (this.chatHistory.length > 50) this.chatHistory.shift(); this.broadcast('chat', chatMsg); }

    stopLoops() {
        clearInterval(this.gameLoop); clearInterval(this.sunLoop); clearInterval(this.brainLoop); clearInterval(this.waveLoop);
        this.gameLoop = null; this.sunLoop = null; this.brainLoop = null; this.waveLoop = null;
    }
    ...
    startLoops() {
        ...
        this.sunLoop = setInterval(() => { if (this.state === 'playing') this.broadcastTo('plants', 'skySun', { x: Math.random() * 700 + 80, y: 40 }); }, 10000);
        this.brainLoop = setInterval(() => { if (this.state === 'playing') this.broadcastTo('zombies', 'skyBrain', { x: Math.random() * 700 + 80, y: 40 }); }, 10000);
        // Auto wave loop - free waves every 45 seconds (zombies can also buy extra waves)
        this.waveLoop = setInterval(() => { if (this.state === 'playing') this.autoWave(); }, 45000);
    }
    ...
    startGame() {
        ...
        setTimeout(() => { if (this.state === 'playing') this.autoWave(); }, 15000);
        this.startBotAI();
    }
    ...
    buyWave(byName) {
        ...
        this.broadcast('waveStart', { waveNumber: this.waveNumber, maxWaves: this.maxWaves, zombieCount: baseCount, brainCount: this.brainCount, by: byName });
    ...
    update() {
        ...
        this.plants.forEach(plant => {
            ...
            if (plant.type === 'sunflower') {
                ...
                if (plant.sunTimer >= PLANT_STATS.sunflower.sunInterval) { plant.sunTimer = 0; this.broadcastTo('plants', 'plantSun', { col: plant.col, row: plant.row }); }
            }
            ...
            if (plant.type === 'peashooter' || plant.type === 'snowpea' || plant.type === 'repeater') {
                ...
                if (plant.shootTimer >= stats.shootInterval) {
                    if (this.zombies.some(z => z.row === plant.row && z.x > plant.col * CELL_SIZE && z.hp > 0)) {
                        ...
                        const pea = { id: 'p_' + Date.now() + Math.random().toString(36).substr(2,4), x: plant.col * CELL_SIZE + 80, y: plant.row * CELL_SIZE + 40, row: plant.row, damage: stats.damage, slows: plant.type === 'snowpea', type: peaType };
                        ...
                        if (plant.type === 'repeater') {
                            setTimeout(() => {
                                ...
                                const pea2 = { id: 'p_' + Date.now() + Math.random().toString(36).substr(2,4), x: plant.col * CELL_SIZE + 80, y: plant.row * CELL_SIZE + 40, row: plant.row, damage: stats.damage, slows: false, type: 'normal' };
            ...
            if (plant.type === 'chomper' && !plant.cooldown) {
                ...
                if (nearby) { nearby.hp = 0; plant.cooldown = true; this.broadcast('chomperEat', { col: plant.col, row: plant.row, zombieId: nearby.id }); setTimeout(() => { plant.cooldown = false; this.broadcast('chomperReady', { col: plant.col, row: plant.row }); }, PLANT_STATS.chomper.eatCooldown); }
            }
            ...
            if (plant.type === 'potatomine' && plant.armed) {
                const nearby = this.zombies.find(z => z.row === plant.row && Math.abs(z.x - plant.col * CELL_SIZE) < 80 && z.hp > 0);
                if (nearby) { 
                    this.zombies.forEach(z => { if (z.row === plant.row && Math.abs(z.x - plant.col * CELL_SIZE) < 150) z.hp -= PLANT_STATS.potatomine.damage; }); 
                    plant.hp = 0; 
        ...
        this.zombies.forEach(zombie => {
            ...
            if (targetPlant && ZOMBIE_STATS[zombie.type].damage > 0 && !zombie.canJump) {
                ...
                if (zombie.eatTimer >= 800) { zombie.eatTimer = 0; targetPlant.hp -= ZOMBIE_STATS[zombie.type].damage; this.broadcast('plantDamage', { col: targetPlant.col, row: targetPlant.row, hp: targetPlant.hp }); if (targetPlant.hp <= 0) { this.grid[targetPlant.col][targetPlant.row] = null; this.broadcast('plantDie', { col: targetPlant.col, row: targetPlant.row }); } }
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (!pea.fire) {
                const torch = this.plants.find(p => p.type === 'torchwood' && p.row === pea.row && p.hp > 0 && Math.abs(p.col * CELL_SIZE + 50 - pea.x) < 30);
                if (torch) {
                    pea.fire = true;
                    pea.damage *= 2;
            ...
            if (hit) { hit.hp -= pea.damage; if (pea.slows && !hit.slowed) { hit.slowed = true; hit.speed *= 0.5; } this.broadcast('peaHit', { peaId: pea.id, zombieId: hit.id, zombieHp: hit.hp, slowed: pea.slows, fire: pea.fire }); this.projectiles.splice(i, 1); }
            else if (pea.x > 1100) { this.broadcast('peaMiss', { peaId: pea.id }); this.projectiles.splice(i, 1); }
        }
        ...
        if (!this.lastBroadcast || Date.now() - this.lastBroadcast > 50) {
            ...
            this.broadcast('gameUpdate', { zombies: this.zombies.map(z => ({ id: z.id, x: z.x, hp: z.hp, slowed: z.slowed })), plants: this.plants.map(p => ({ col: p.col, row: p.row, hp: p.hp })), sunCount: this.sunCount, brainCount: this.brainCount, waveNumber: this.waveNumber });
        }
    }
    ...
    placePlant(type, col, row, byName) {
        ...
        this.plants.push(plant); this.grid[col][row] = plant;
        this.broadcast('plantPlaced', { type, col, row, hp: plant.hp, maxHp: plant.maxHp, by: byName, sunCount: this.sunCount, rechargeMs: stats.recharge });
        if (type === 'potatomine') setTimeout(() => { if (plant.hp > 0) { plant.armed = true; this.broadcast('mineArmed', { col, row }); } }, PLANT_STATS.potatomine.armTime);
        if (type === 'cherrybomb') setTimeout(() => { if (plant.hp > 0) { const cx = col * CELL_SIZE + 50, cy = row * CELL_SIZE + 50; this.zombies.forEach(z => { if (Math.hypot(z.x - cx, z.row * CELL_SIZE - cy) < PLANT_STATS.cherrybomb.radius) z.hp = 0; }); plant.hp = 0; this.grid[col][row] = null; this.broadcast('cherryExplode', { col, row }); } }, PLANT_STATS.cherrybomb.explodeTime);
        return true;
    }
    ...
    removePlant(col, row, byName) {
        ...
        plant.hp = 0; this.grid[col][row] = null;
    ...
    spawnZombie(type, row, byName) {
        ...
        const zombie = { id: 'z_' + Date.now() + Math.random().toString(36).substr(2, 4), type, row, x: 1000, hp: stats.hp, maxHp: stats.hp, speed: stats.speed, slowed: false };
        if (type === 'polevaulter') zombie.canJump = true;
        this.zombies.push(zombie);
        this.broadcast('zombieSpawned', { id: zombie.id, type, row, hp: zombie.hp, maxHp: zombie.maxHp, by: byName, brainCount: this.brainCount, rechargeMs: isAutoWave ? 0 : stats.recharge });
        return true;
    }

    collectSun(amount) { this.sunCount = Math.min(9999, this.sunCount + amount); this.broadcast('sunUpdate', { sunCount: this.sunCount }); }
    collectBrain(amount) { this.brainCount = Math.min(9999, this.brainCount + amount); this.broadcast('brainUpdate', { brainCount: this.brainCount }); }
    ...
    // Smarter AI
    startBotAI() {
        this.plantPlayers.filter(p => p.isBot).forEach(bot => {
        ...
        this.zombiePlayers.filter(p => p.isBot).forEach(bot => {
...
Object.entries(persistedData.roomStates || {}).forEach(([id, saved]) => {
    ...
    saved.plantPlayers.forEach(p => { room.plantPlayers.push({ ...p, socket: null }); });
    saved.zombiePlayers.forEach(p => { room.zombiePlayers.push({ ...p, socket: null }); });
    rooms.set(id, room);
    if (room.state === 'playing') room.startLoops();
});

io.on('connection', (socket) => {
    let currentRoom = null, playerName = '玩家', currentTeam = null, oderId = null;

    socket.on('restore', ({ oderId: oid, name }, callback) => {
        oderId = oid; playerName = name || '玩家';
        ...
        if (sess && sess.roomId) {
            ...
            if (room) {
                ...
                if (player) {
                    player.socket = socket; player.id = socket.id; player.name = playerName;
                    currentRoom = room; currentTeam = room.plantPlayers.includes(player) ? 'plants' : 'zombies';
                    ...
                    callback({ restored: true, roomId: room.id, team: currentTeam, mode: room.mode, maxWaves: room.maxWaves, state: room.state, playerList: room.getPlayerList(), gameState: room.getGameState(), chatHistory: room.chatHistory.slice(-20) });
    ...
    socket.on('setName', (name) => { playerName = name || '玩家'; if (currentRoom) { const player = currentRoom.findPlayerByOderId(oderId); if (player) player.name = playerName; } });
    socket.on('listRooms', () => { const list = []; rooms.forEach(room => { if (room.state === 'waiting' || room.state === 'paused') list.push(room.getInfo()); }); socket.emit('roomList', list); });
    socket.on('getLeaderboard', () => { socket.emit('leaderboard', persistedData.leaderboard.slice(-20).reverse()); });
    ...
    socket.on('joinRoom', ({ roomId, team }, callback) => {
        ...
        currentRoom = room; currentTeam = team;
        sessions.set(oderId, { roomId, team, name: playerName }); saveData();
        callback({ success: true, team, mode: room.mode, maxWaves: room.maxWaves, state: room.state, playerList: room.getPlayerList(), gameState: room.getGameState() });
    ...
    socket.on('addBot', ({ team }, callback) => { if (!currentRoom) return callback({ success: false }); const bot = currentRoom.addBot(team); if (!bot) return callback({ success: false, error: '队伍已满' }); currentRoom.broadcast('playerUpdate', { playerList: currentRoom.getPlayerList(), info: currentRoom.getInfo() }); callback({ success: true }); if (currentRoom.isFull() && currentRoom.state === 'waiting') currentRoom.startGame(); });
    socket.on('placePlant', ({ type, col, row }) => { if (currentRoom && currentTeam === 'plants') currentRoom.placePlant(type, col, row, playerName); });
    socket.on('removePlant', ({ col, row }) => { if (currentRoom && currentTeam === 'plants') currentRoom.removePlant(col, row, playerName); });
    socket.on('spawnZombie', ({ type, row }) => { if (currentRoom && currentTeam === 'zombies') currentRoom.spawnZombie(type, row, playerName); });
    socket.on('buyWave', () => { if (currentRoom && currentTeam === 'zombies') currentRoom.buyWave(playerName); });
    socket.on('collectSun', ({ amount }) => { if (currentRoom && currentTeam === 'plants') currentRoom.collectSun(amount || 25); });
    socket.on('collectBrain', ({ amount }) => { if (currentRoom && currentTeam === 'zombies') currentRoom.collectBrain(amount || 25); });
    socket.on('chat', ({ message }) => { if (currentRoom && message && message.trim()) currentRoom.sendChat(playerName, message.trim().substring(0, 100)); });
    ...
    socket.on('leaveGame', (forceLeave) => {
        ...
        if (oderId) { sessions.delete(oderId); saveData(); }
    });
    ...
    socket.on('disconnect', () => { console.log(`[Disconnect] ${playerName}`); });
});
